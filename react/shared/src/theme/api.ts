import { ServiceInstance as _Service } from "../services/Service";
...
export const load = async (theme: string): Promise<ThemeResult | null> => {
...
export async function getTheme(): Promise<Record<string, unknown>> {
  const info = await _Service.info() as Record<string, unknown>;
  return info;
}

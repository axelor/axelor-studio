...
const LoadingAnimation = ({ loadingTexts }: LoadingAnimationProps) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setIsFadingOut(true);
      setTimeout(() => {
        setCurrentIndex((currentIndex) => (currentIndex + 1) % loadingTexts.length);
  ...
  return (
    <Box id="page" d="flex" justifyContent="center" bg="body" style={{ height: "90vh" }}>
      <Box
        id="phrase_box"
        color="body"
        pb={4}

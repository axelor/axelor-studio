...
const isSameAsOriginal = (item, originalWidgets) => {
	if (item) {
		...
		const itemExplore = exploreTarget(item.target)
		const widget = Object.values(originalWidgets).find((widget) => {
			const widgetExplore = exploreTarget(widget.xPath)
		...
		const attributeName = dashToCamelCase(attributes.name)
		if (widget) {
...
export function getUpdatedAttrsList(state, originalWidgets) {
	...
	const currentSchema = generateXMLToViewSchema({
	...
	movesMap.forEach((value) => {
		const move = value
		const moveExplore = exploreTarget(move.target)
...
export function optimizeExtension(state, originalWidgets) {
	...
	const filterListForRemove = (move) => {
		...
		const explore = exploreTarget(move.target)

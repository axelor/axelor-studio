...
- *Icon background* (8): Select a single background color for the icon. The field uses a `SingleSelect` widget, so only one color can be applied at a time. The 11 available colors come from the `studio.menu.icon.background.select` selection: `blue`, `black`, `brown`, `green`, `orange`, `pink`, `purple`, `red`, `violet`, `white`, `yellow`.

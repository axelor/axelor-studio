...
- *Icon background* (8): Select a background color for the icon (SingleSelect widget bound to the `studio.menu.icon.background.select` selection). The 11 available colors are: `blue`, `black`, `brown`, `green`, `orange`, `pink`, `purple`, `red`, `violet`, `white`, `yellow`.

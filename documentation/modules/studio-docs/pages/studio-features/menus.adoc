...
- *Icon background* (8): Select a background color for the icon from the `studio.menu.icon.background.select` selection (SingleSelect widget). The 11 available colors are: `blue`, `black`, `brown`, `green`, `orange`, `pink`, `purple`, `red`, `violet`, `white`, `yellow`.

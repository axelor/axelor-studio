...
=== Enable Quick Edit Button (3)

* **Field**: `enableStudioButton`
* **Type**: Boolean switch
* **Default**: `true`

When enabled, a *Studio* button is injected into the toolbar of form views for authorized users, giving them a quick entry point to edit the current view in Studio. When disabled, the button is not added.

The button injection is performed by `ViewProcessorImpl`, which adds the open-Studio button to a form's toolbar only when `getEnableStudioButton()` returns `true`.
...
| Boolean
| `true`
| Adds the quick-edit Studio button to form toolbars for authorized users
|===

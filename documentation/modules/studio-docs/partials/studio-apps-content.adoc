...
| *Meta Actions*
| Generated meta actions (`MetaAction`) associated with this app
...
Two sidebar buttons control whether the app is surfaced in the central
xref:studio-docs:studio-features/apps-management.adoc[Apps Management] view:

* btn:[Add to App Management] -- Shown when the app is not yet listed there
(`isInAppView` is false). It flags the app so it appears as a card in Apps Management.
* btn:[Remove from App Management] -- Shown when the app is already listed
(`isInAppView` is true). It removes the card from Apps Management without deleting the
Studio App itself.
...
The btn:[Update app] action lets you refresh an existing Studio App from an exported
data file without recreating it. It opens an *Update app* dialog:
...
| When checked, elements that exist in the database but are absent from the imported
file are *detached* from the application -- they are unlinked from the app, not
deleted. When unchecked, existing elements are kept even if they are missing from the
file.
|===

...
. Optionally check *Migrate all the instances of previous version* (4): When enabled, every running instance of the source version is migrated; otherwise only the instances explicitly targeted are migrated.
...
After a migration runs, the *Migration report* panel summarizes the outcome:
...
| Number of instances selected for migration.

| Successful migrations
| Number of instances migrated without error.

| Failed migrations
| Number of instances whose migration raised an error.
|===

When *Failed migrations* is greater than zero, an *Instances not migrated due to error* button appears. It opens the *Process Instance* grid filtered on the instances of the source model whose `migrationStatusSelect` equals `3` (migration error), so the failures can be inspected and re-processed.
...
To prevent concurrent migrations of the same model, a deployment that carries a migration acquires a lock on the target model through the `WkfModel.isMigrationOnGoing` flag:

* When such a deployment starts, the flag is set to `true` on the target model.
* If another deployment with migration is attempted while the flag is still set, it is rejected with the error _"Migration is already ongoing."_
* When the deployment completes -- successfully or with an error -- the flag is reset to `false`, so the lock is released automatically even when the migration fails.
...
| `upgradeAllInstances`
| Boolean
| Whether to migrate all running instances of the previous version
...
| Integer
| Number of instances selected for migration (report)

| `successfulMigrations`
| Integer
| Number of instances migrated without error (report)

| `failedMigrations`
| Integer
| Number of instances whose migration raised an error (report)
|===

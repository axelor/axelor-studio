...
After the migration runs, the *Migration report* panel of the migration form
summarizes the outcome across all instances that were processed:
...
| Number of running instances that were targeted by the migration.

| *Successful migrations*
| Number of instances that were migrated without error.

| *Failed migrations*
| Number of instances whose migration raised an error.

| btn:[Show errored instances]
| Button shown only when *Failed migrations* is greater than `0`. It opens the list
of instances whose `migrationStatusSelect` is *Migration error*, so you can inspect
and retry them individually.
|===

These counters are computed by `WkfMigrationServiceImpl` from the migration map
returned by the deployment pipeline (`totalInstancesToMigrate`,
`successfulMigrations`, `failedMigrations`) and persisted on the `WkfMigration`
record.
...
To prevent two migrations of the same model from running at the same time, each
`WkfModel` carries an `isMigrationOnGoing` flag:

* When a deployment/migration starts, the flag is set to `true` on the target model.
* If a new deployment is requested while the flag is already `true`, the request is
rejected with the message *"Migration is already ongoing."*
* On normal completion, the flag is reset to `false`.
* If the migration fails with an exception, the flag is also automatically reset to
`false` (in the controller's error handling) so that the model is not left
permanently locked, and a BPM error message is sent.

This guarantees that a failed or interrupted migration does not block future
...
| Integer
| Number of instances targeted by the migration

| `successfulMigrations`
| Integer
| Number of instances migrated successfully

| `failedMigrations`
| Integer
| Number of instances whose migration failed
|===
...
* `BpmDeploymentServiceImpl.setIsMigrationOnGoing()` -- Sets/clears the `WkfModel.isMigrationOnGoing` concurrency lock

...
To prevent two deployments (and their associated migrations) of the same BPM model from running at the same time, each model carries a migration lock. When a deployment starts, the lock is raised on the target model; it is released once the deployment finishes.

If you try to deploy a model while a migration is already in progress for it, the operation is rejected and the following message is shown:

----
Migration is already ongoing.
----

The lock behaves as follows:

* It is raised at the beginning of the deployment of the target model.
* It is released automatically when the deployment completes, whether or not a migration map was provided.
* If the deployment fails with an error, the lock is reset automatically so that the model can be deployed again. The error is also reported through the BPM error message channel.

This guard applies to the model being deployed; deployments of other (unrelated) models are not blocked.

// TODO-RECAPTURE 3.3 @8-2-preview: BPM model deploy attempt while a migration is ongoing, showing the "Migration is already ongoing." error -> modules/bpm-docs/assets/images/migration/migration-ongoing-error.png
...
* `WkfModelController.deploy()` -- Enforces the concurrent migration guard via the `isMigrationOnGoing` flag and resets it on error
* `BpmDeploymentServiceImpl.setIsMigrationOnGoing()` -- Raises and releases the migration lock on the model

...
After a migration runs, the form displays a *Migration report* panel summarizing the outcome over all targeted instances:
...
| Number of running instances that were selected for migration (`totalInstancesToMigrate`).

| *Successful migrations*
| Number of instances migrated without error (`successfulMigrations`).

| *Failed migrations*
| Number of instances whose migration failed (`failedMigrations`).
|===

When at least one instance failed, an btn:[Instances not migrated due to error] button appears, opening the list of instances that could not be migrated so the cause can be investigated.
...
To prevent two migrations from running against the same model at the same time, the BPM model carries an `isMigrationOnGoing` flag (`WkfModel.isMigrationOnGoing`). While a migration is in progress the flag is set; if a second migration is requested for the same model, the engine rejects it with the message *"Migration is already ongoing."* The flag is reset automatically once the migration completes (whether it succeeds or fails).
...
| Integer
| Number of instances selected for migration (migration report)

| `successfulMigrations`
| Integer
| Number of instances migrated successfully (migration report)

| `failedMigrations`
| Integer
| Number of instances that failed to migrate (migration report)
|===

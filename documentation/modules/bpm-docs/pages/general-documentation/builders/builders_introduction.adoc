...
The server-side generation of Groovy scripts and expressions is performed by dedicated services and Groovy templates:

[cols="1,2", options="header"]
|===
| Component | Role

| `GroovyExpressionBuilderController`
| Controller exposing the generation endpoints `buildExpression` (condition / expression output) and `buildScript` (mapper / record-manipulation output) to the builders.

| `GroovyScriptBuilderService` / `GroovyScriptBuilderServiceImpl`
| Parses the JSON metadata (`ScriptDto`, `RuleDto`, ...) and renders the corresponding Groovy template into the final script.

| Groovy templates (`templates/groovy*.tmpl`)
| The text templates used for generation -- `groovyScript.tmpl` (mapper / record updates), `groovyExpression.tmpl` and `groovyCondition.tmpl` (conditions), `groovyRule.tmpl` (rule groups), `groovyFieldName.tmpl` (field paths).
|===

The generated output is a Groovy script or expression; the same scripting context and helpers described in <<Scripting>> apply to it.

...
For integration within other React applications (such as the BPM modeler), builders are imported directly as React components:
...
This pattern is used by the BPM modeler for task configuration.
...
| BPM task configuration (script tasks, user tasks), Studio Actions (Create/Update), xref:general-documentation/utilities/value_mapper.adoc[Value Mapper]
...
| BPM task queries (fetch data from database)

...
When the auto-migration step migrates running instances, the engine can report
real-time progress to the user interface so that long migrations no longer appear
as a frozen spinner. This behavior is controlled by the
xref:general-documentation/app-management/app_bpm_configuration.adoc[*Enable Using
progress deployment display bar*] flag (`AppBpm.useProgressDeploymentBar`, default
`true`).

=== How Progress Is Reported

When the flag is enabled, the deployment pipeline pushes progress updates over a
WebSocket channel:

. The BPM modeler (React application) opens a WebSocket to the `/bpm/deploy/progress`
endpoint, passing the model id as the `customId` query parameter.
. As each running instance is migrated, the backend computes a percentage
(`migrated instances / total instances to migrate`) and pushes it to the matching
WebSocket session.
. The modeler displays a progress bar that advances as the percentage increases, and
switches to a completed state when the migration finishes.

The percentage is computed by `BpmDeploymentServiceImpl.calculatePercentage()` and
sent through `BpmDeploymentWebSocket.updateProgress()`, which locates the WebSocket
session whose key matches the deployed model id.

[NOTE]
====
The progress bar reports the *instance migration* phase of a redeployment. When a
model is deployed for the first time (no running instances to migrate), the
deployment completes without intermediate progress steps.
====

When the flag is disabled, deployment runs exactly the same way but no WebSocket
progress messages are sent; the UI simply shows a standard loading indicator until
deployment finishes.
...
* `BpmDeploymentServiceImpl.calculatePercentage()` -- Computes the migration progress percentage
* `BpmDeploymentWebSocket` -- WebSocket endpoint (`/bpm/deploy/progress`) that pushes deployment progress to the modeler

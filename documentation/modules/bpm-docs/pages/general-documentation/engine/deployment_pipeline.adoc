...
When the *Enable Using progress deployment display bar* flag is active on the App BPM Configuration (`AppBpm.useProgressDeploymentBar`, see xref:general-documentation/app-management/app_bpm_configuration.adoc[App BPM Configuration]), the modeler shows a real-time progress bar while a model is being deployed.

Progress is delivered over a WebSocket channel exposed by the BPM module at the endpoint `/bpm/deploy/progress` (`BpmDeploymentWebSocket`). As the deployment pipeline advances through its steps, the backend publishes the completion percentage to the client session through `BpmDeploymentWebSocket.updateProgress()`. The front-end subscribes to this channel and updates the progress bar accordingly.

When the flag is disabled, deployment runs exactly the same way but without emitting progress events, and the modeler shows no progress bar.
...
* `BpmDeploymentWebSocket` -- WebSocket endpoint (`/bpm/deploy/progress`) that streams deployment progress to the modeler

...
When a new version is deployed and running instances are auto-migrated, the BPM modeler displays a real-time progress overlay. The progress reflects the per-instance migration loop: as each running instance is migrated to the new version, the completion percentage is pushed to the modeler and the overlay updates accordingly.

// TODO-RECAPTURE 3.3 @8-2-preview: BPM modeler showing the migration progress overlay ("NN% migration is done...") during deployment of a new version with running instances -> modules/bpm-docs/assets/images/engine/deployment-progress-overlay.png

=== How It Works

The progress is delivered over a WebSocket channel rather than a regular HTTP request, so the modeler receives incremental updates without polling:

. When the *Deploy* button is clicked, the modeler opens a WebSocket connection to the `/bpm/deploy/progress` endpoint.
. During the per-instance migration loop, the backend computes the completion percentage and pushes it to the connected session.
. The modeler updates the progress overlay each time a new percentage is received.
. Once the value reaches `100`, the overlay is automatically dismissed shortly after completion.

The percentage is computed as `(migratedInstances / totalInstances) * 100` (`BpmDeploymentServiceImpl.calculatePercentage()`), so it is only meaningful when there are running instances to migrate. A deployment without auto-migration completes without showing intermediate progress steps.

=== Technical Details

* The WebSocket endpoint is implemented by `BpmDeploymentWebSocket`, registered at `/bpm/deploy/progress`. It keeps a map of open sessions and the last known progress per session.
* During auto-migration, `BpmDeploymentServiceImpl` calls `BpmDeploymentWebSocket.updateProgress()` after each instance is migrated, sending the current percentage to the open session.
* On the client side, the `wsProgress` service (`react/bpm/src/services/Progress.js`) opens the WebSocket, parses incoming messages, and notifies subscribers. The BPM modeler subscribes to these updates and renders the progress overlay.

...
When a model is deployed from the BPM modeler, the server can report deployment progress in real time so the modeler can display a progress bar instead of an indeterminate spinner.

This is driven by a WebSocket endpoint, `BpmDeploymentWebSocket`, registered at `/bpm/deploy/progress`:

* The modeler opens the WebSocket with a `customId` query parameter that identifies the deployment session.
* As deployment advances, the server pushes a percentage to the client via `BpmDeploymentWebSocket.updateProgress(sessionId, percentage)`.
* On connection (and on each client message), the endpoint replies with the current progress so a reconnecting client can resynchronise.

The progress bar is shown only when *Enable Using progress deployment display bar* (`useProgressDeploymentBar`) is enabled in the xref:general-documentation/app-management/app_bpm_configuration.adoc#_migration_panel[App BPM Configuration]; it is enabled by default. The instance-migration phase of a deployment contributes up to a bounded share of the overall progress (the migration phase is capped at a fixed percentage of the bar).

The client side is implemented in `react/bpm/src/services/Progress.js`.

// TODO-RECAPTURE 3.6 @bpm: deployment progress bar shown in the BPM modeler during deploy -> assets/images/engine/deployment-progress-bar.png
...
* `BpmDeploymentWebSocket` -- WebSocket endpoint (`/bpm/deploy/progress`) pushing deployment progress to the modeler

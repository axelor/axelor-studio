...
While a model is being deployed, the BPM modeler can display a live progress bar that reflects how far the deployment pipeline has advanced. The backend publishes progress events over a WebSocket endpoint and the React modeler renders them as an overlay.

* *Backend*: `BpmProgressWebSocket` exposes the WebSocket endpoint `/bpm/deploy/progress`. As the deployment pipeline runs, it pushes an integer `progress` value (0-100) to the connected client, keyed by a custom session id passed as the `customId` query parameter.
* *Frontend*: the modeler (`Progress.ts` / `ProgressOverlay.tsx`) subscribes to the endpoint and shows the progress overlay.

The progress bar is controlled by the *Enable Using progress deployment display bar* flag on the xref:general-documentation/app-management/app_bpm_configuration.adoc[App BPM Configuration] (`AppBpm.useProgressDeploymentBar`, enabled by default). When the flag is disabled, deployment still runs but no progress overlay is shown.
...
== Camunda Identity Synchronization

When a model is deployed, Axelor users and roles are synchronized to the embedded Camunda Identity Service so that user tasks can be assigned and claimed correctly. This synchronization is triggered automatically as part of the deployment pipeline (`BpmDeploymentServiceImpl` calls `WkfIdentityService.syncModelIdentities()`).

The mapping is driven by the BPM model's notification configuration:

* *Admin Users* and *Admin Roles* are mapped to the Camunda *admin* group (full access).
* *Users* and *User Roles* are mapped to the Camunda *user* group (standard access).

=== Manual Re-synchronization

A deployed model (status *On going*) exposes a *Camunda Identity Sync* panel on its form with a btn:[Synchronize Identities to Camunda] button. Clicking it re-runs the synchronization for that model via `WkfModelController.resyncIdentities()` -> `WkfIdentityService.resyncModelIdentities()`.

The outcome is recorded on the model and shown in a *Synchronization Report* panel:
...
| `syncStatus`
| Result of the last synchronization: *Pending* (0), *Success* (1), or *Error* (2) (`wkf.sync.status.select`).

| `lastSyncDate`
| Timestamp of the last synchronization.

| `syncLog`
| Detailed log of the synchronization (read-only text).
|===

Authorization of who may act on a deployed model and its user tasks is handled by the `com.axelor.studio.bpm.service.authorization` services (`BpmPermissionService`, `BpmAuthorizationService`).

== Real-time Collaboration

The BPM modeler supports real-time collaboration so that several users can open the same model concurrently. The modeler tracks who is currently editing a model, whether the model is dirty, and surfaces the list of connected editors.

* *Frontend*: `Collaboration.tsx` displays the connected users; `collaboration.service.ts` maintains the room state (users, dirty flag, versions) over a socket channel.

This prevents silent overwrites when two users edit and deploy the same BPM model.

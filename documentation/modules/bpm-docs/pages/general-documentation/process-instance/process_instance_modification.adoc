...
== Instance Status and Timer-Blocked Instances

Beyond ad-hoc modification scripts, the *Process Instances* grid (*BPM components > Technical monitoring > Process Instances*) lets administrators monitor and unblock running instances.

=== Instance Status

Each `WkfInstance` carries a *Status* field (`statusSelect`) that reflects whether the instance is still advancing:
...
| *Stopped* (2)
| The instance is no longer advancing (for example, it is waiting on a timer or has been halted).
|===

The status is shown as a column on the Process Instances grid.

=== Timer-Blocked Instances

Instances that are waiting on a timer event can be inspected and resumed from the grid's *Timer-Blocked instances* menu:

* btn:[Show timer-blocked instances] -- filters the grid to instances currently blocked on a timer (`WkfInstanceController.showTimerBlockedInstances()`).
* btn:[Resume timer-blocked instances] -- resumes processing for the selected timer-blocked instances (`WkfInstanceController.resumeTimerBlockedInstances()`). At least one timer-blocked instance must be selected.

From an individual instance form, the btn:[Unblock process instance] button performs the same resume operation for that single instance.

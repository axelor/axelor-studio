...
In the xref:general-documentation/web-services/ws-builder.adoc[WS Builder], the *Transformation Builder* sub-tool lets you assemble transformations visually:

. Select a *library*
. Select a *transformation* (operation) from that library
. Configure the operation's *parameters*

// TODO-RECAPTURE 3.4 @generic-builder: WS Builder Transformation Builder panel (library + operation + parameters) -> web-services/ws-transformation-builder.png

Several transformation rows can be chained: each row applies its operation to the result of the previous one, and rows can be added or removed from the panel. The composed chain is resolved on the backend by `FullTransformation`, which substitutes the `#\{target}` and `#\{paramName}` placeholders of each transformation's Groovy template and compiles the combined script through `GroovyShell`.

The resulting transformation is then applied to the data flow within the web service workflow.

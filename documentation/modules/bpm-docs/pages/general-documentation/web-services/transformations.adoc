...
Inside the shared builders, a *Data transformation* dialog lets you apply one or more transformations to a value being mapped. You add a transformation row (btn:[Add Filter]), pick a *library* and an *operation* (transformation) from that library, and fill in its parameters. Multiple transformations can be stacked so the output of one becomes the input (`target`) of the next, building a transformation chain on a single value.

The dialog produces a call to `__ctx__.transform(name, library, [...])` from the `transformation.tmpl` template; nested transformations are emitted by `groovyScriptService.buildTransformation()`, and at runtime each call is resolved by `FullTransformation`, which substitutes the transformation's Groovy template parameters and evaluates the result.

This dialog is the React component `TransformationBuilder` (in `react/generic-builder/`), embedded by the builders rather than opened from a menu.

// TODO-RECAPTURE 3.6 @generic-builder: Data transformation dialog (Add Filter + library/operation/parameters) -> assets/images/web-services/data-transformation-dialog.png
